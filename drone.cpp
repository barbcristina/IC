                    ...
                    if (pontoA >= 0 && pontoA < n && pontoB >= 0 && pontoB < n && i >= 0 && i < n) {
                        ...
                        if (std::isfinite(distancias[pontoA][i]) && std::isfinite(distancias[i][pontoB])) {
                            ...
                            if (std::isfinite(distancias[pontoA][pontoB])) {
                                custoInsercao = (distancias[pontoA][i] + q[pontoC][pontoA][i]) + (distancias[i][pontoB] + q[pontoA][i][pontoB]) + q[i][pontoB][pontoD] - (distancias[pontoA][pontoB] + q[pontoC][pontoA][pontoB]);
                            }
...
int main() {
    ...
    std::string rotasG = "rotagurobi.txt";
    std::ofstream resultadosFile("resultados.txt");

    // Abrir o arquivo rotasG para leitura
    std::ifstream arquivoRotas(rotasG);
    if (!arquivoRotas.is_open()) {
        std::cerr << "Erro ao abrir o arquivo de rotas." << std::endl;
        return 1;
    }

    // Ler todas as linhas do arquivo rotasG
    std::vector<std::string> linhasRotas;
    std::string linhaRota;
    while (std::getline(arquivoRotas, linhaRota)) {
        linhasRotas.push_back(linhaRota);
    }
    arquivoRotas.close();
    ...
    for(int nObs = 10; nObs > 0; nObs--){
        ...
        // Usar uma linha do arquivo rotasG para representar um vector
        //std::istringstream issRota(linhasRotas[10 - nObs]);
        //std::vector<int> cicloHamiltoniano;
        //int valor;
        //while (issRota >> valor) {
        //    cicloHamiltoniano.push_back(valor);
        //}
        ...
        fronteira = {0, 1, 2, 3, 17, 32, 33, 34, 21, 8, 9, 10, 11, 26, 41, 55, 69, 83, 97, 96, 95, 94, 107, 121, 134, 133, 146, 161, 162, 163, 164, 165, 166, 167, 181, 195, 194, 193, 192, 191, 190, 175, 174, 173, 172, 171, 170, 169, 154, 140, 141, 142, 143, 130, 115, 114, 113, 112, 98, 84, 71, 57, 43, 28, 14};
