...
int encontrarProximoPonto(int anterior, int atual, const std::vector<std::vector<double>>& distancias, std::vector<std::vector<std::vector<double>>>& q, std::vector<bool>& visitado, int pontoInicial, std::vector<int> ciclo) {
    int n = distancias.size();
    int proximoPonto = -1;
    double menorDistancia = std::numeric_limits<double>::max();

    for (int i = 0; i < n; i++) {
        if (i != atual && !visitado[i] && distancias[atual][i] + q[anterior][atual][i] < menorDistancia) {
            proximoPonto = i;
...
std::vector<int> KNN2INI(int tam, const std::vector<std::vector<double>>& distancias, std::vector<std::vector<std::vector<double>>>& q, int obsSize) {
    ...
    for (int i = 1; i < n/2; i++) {
        int proximoPonto = encontrarProximoPonto(pontoantant, pontoAtual, distancias, q, visitado, pontoInicial, cicloHamiltoniano);
    ...
    for (int i = n/2; i < n; i++) {
        int proximoPonto = encontrarProximoPonto(pontoant, pontoat, distancias, q, visitado, pontoini, cicloHamiltoniano);
...
int main() {
    ...
    std::vector<int> melhorRota = grasp(validos.back(), distancias, q, altitudes, obstaculos_indices.size());
